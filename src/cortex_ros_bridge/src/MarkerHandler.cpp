    ...
    */
	...
	for ( int i = 0; i < FrameOfData->nBodies; ++i )
	{
		for ( int j = 0; j < FrameOfData->BodyData[i].nMarkers; ++j )
		{
			...
			// set color
			int colorSelector = i%3;
			int colorDivider = floor(i/3);
			marker_tmp.color.r = colorSelector==0?(255.0/(colorDivider+1)):0;
			marker_tmp.color.g = colorSelector==1?(255.0/(colorDivider+1)):0;
			marker_tmp.color.b = colorSelector==2?(255.0/(colorDivider+1)):0;
